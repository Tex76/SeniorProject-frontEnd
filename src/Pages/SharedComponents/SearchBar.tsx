...
import { InputBase, lighten, Box, Avatar, ListItem, ListItemAvatar, ListItemText } from '@mui/material';
...
export default function SearchBar() {
  ...
  const filteredData = data.map(category => ({
    ...category,
    items: category.items.filter(item => item.title.toLowerCase().includes(searchInput.toLowerCase()))
  })).filter(category => category.items.length > 0);

  return (
    <Search sx={{ backgroundColor: "#E4E4E4", borderRadius: "30px", position: 'relative' }}>
      <SearchIconWrapper>
        ...
        <Box sx={{ 
         ...
         {filteredData.map((category, index) => (
            <Box key={index} sx={{ borderBottom: '1px solid #ddd'}}>
              <Box sx={{ p: 2, fontWeight: 'bold', color: 'white', backgroundColor: 'teal', fontFamily: 'Roboto', fontSize: '18px'}}>{category.category}</Box>
              {category.items.map((item, index) => (
                <Link to={item.link} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <ListItem key={index} sx={{ borderBottom: '1px solid #ddd', '&:hover': { backgroundColor: 'lightgrey' } }}>
                    <ListItemAvatar>
                      <Avatar alt={item.title} src={item.image} sx={{ width: '64px', height: '48px', borderRadius: '10px', mr: '20px' }} />
                    </ListItemAvatar>
                    <ListItemText primary={item.title} primaryTypographyProps={{ color: 'black', fontFamily: 'Roboto', fontSize: '14px', fontWeight: '500' }} />
                  </ListItem>
                </Link>
              ))}
            </Box>
