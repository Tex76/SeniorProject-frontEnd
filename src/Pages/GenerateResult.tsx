import { Accordion, AccordionSummary, AccordionDetails, Typography, Box, Rating, IconButton, Divider, Grid, Stepper, Step, StepLabel } from "@mui/material";
...
const GenerateResult = () => {
    ...
    <Box>
        ...
        <Typography variant="h3" align="center">Bahrain Trip</Typography>
        <Divider sx={{ mb:10}}/>
        ...
        <Grid container spacing={2}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 'bold', marginBottom: '20px', fontFamily: 'Roboto' }}>Your trip to {country} for {numberOfDays} days</Typography>
            <Typography variant="body1" sx={{ fontFamily: 'Roboto', marginBottom: '20px'}}>Bahrain is a vibrant city that offers a perfect blend of hidden gems, historic landmarks, and great food. As you explore the city, you'll come across hidden corners filled with local charm and unique experiences. Discover ancient ruins and impressive architecture that tell the tales of Bahrain's rich history. Indulge in a culinary adventure as you savor delicious Turkish cuisine, from traditional street food to exquisite restaurants. With three days to explore, you'll have the opportunity to immerse yourself in the city's diverse culture, visit iconic landmarks, and uncover the hidden treasures that make Bahrain a remarkable destination.</Typography>
                        ...
                        <Step key={step} active>
                            ...
                            <Accordion>
                                ...
                                <Box>
                                    ...
                                    <img src={activity.imageUrl} alt={activity.title} style={{ width: '500px', height:'300px' }} />
                                    {/* Favorite button at the top right corner of the image */}
                                    <IconButton sx={{ position: 'absolute', top: 0, right: 0 }}><FavoriteBorderIcon /></IconButton>
                                    </Box>
                                    {/* Display the title of the activity */}
                                    <Typography variant="h6">{activity.title}</Typography>
                                    {/* Display a rating for the activity */}
                                    <Rating name={`rating-${step}`} value={activity.rating} readOnly />
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    {/* Display the type of the activity */}
                                    {activity.type === 'Restaurant' && <RestaurantIcon />}
                                    {activity.type === 'Landmark' && <LandmarkIcon />}
                                    ...
                                    <MonetizationOnIcon />
                                    <Typography>Budget: ${activity.budget}</Typography>
                                    </Box>
