...
import { Button, Modal, Box, Typography, Divider, IconButton, useMediaQuery, useTheme, Card, CardContent, CardMedia, ButtonBase } from '@mui/material';
...
const SavePopUp = () => {
  ...
  return (
    <div>
      <Button onClick={handleOpen} style={{ color: "black" }}>Save</Button>
      <Modal
        open={open}
        onClose={handleClose}
      ...
      >
        <Box sx={style}>
          ...
          <Typography id="save-to-trip" variant="h5" component="h2" sx={{ fontFamily: 'Roboto', fontWeight: 'bold', fontSize: 25 }}>
            Save to trip
          </Typography>
          {/* Map over the cards array to create a Card component for each object */}
            {cards.map((card, index) => (
            <Link to="#" style={{ textDecoration: 'none' }}>
            <Card key={index} sx={{ display: 'flex', backgroundColor: 'rgba(255, 211, 52, 0.58)', mt: 2, p: 2, transition: 'transform 0.3s', '&:hover': { transform: 'scale(1.05)' } }}>
            <CardMedia
                component="img"
              ...
              <CardContent sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <Typography variant="h6" component="div" sx={{ fontFamily: 'Roboto', fontWeight: 'bold', fontSize: 14 }}>
                  {card.tripName}
                </Typography>
